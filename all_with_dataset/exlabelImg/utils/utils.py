...
def prepare_img(filename, img_size=32*16, color=(127.5, 127.5, 127.5), diff_multi = 1):
    import cv2
    import numpy as np
    import torch
    ...
    if diff_multi > 1:
        img_scale_diff(pic, filename, diff_multiplier = diff_multi )

    #print(pic)
    shape = pic.shape[:2]  # shape = [height, width]
    ratio = float(img_size) / max(shape)  # ratio  = old / new
    new_shape = (round(shape[1] * ratio), round(shape[0] * ratio))
    dw = (img_size - new_shape[0]) / 2  # width padding
    dh = (img_size - new_shape[1]) / 2  # height padding
    top, bottom = round(dh - 0.1), round(dh + 0.1)
    left, right = round(dw - 0.1), round(dw + 0.1)
    img = cv2.resize(pic, new_shape, interpolation=cv2.INTER_AREA)  # resized, no border
...
def inference_nms(model, x, pic, conf_thres=0.5, nms_thres=0.3, img_size=32*13):
    pred = model(x)
    ...
    scale_coords(img_size, detections[:, :4], pic.shape).round()
